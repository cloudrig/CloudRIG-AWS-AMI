...
{   Write-Host "$global:CloudRIGInstallBaseDir"
...
Function Install-TightVNC
{
    ...
    (New-Object System.Net.WebClient).DownloadFile($(((Invoke-WebRequest -Uri https://www.tightvnc.com/download.php -UseBasicParsing).Links.OuterHTML -like "*Installer for Windows (64-bit)*").split('"')[1].split('"')[0]), "$global:CloudRIGInstallBaseDir\Apps\tightvnc.msi")
    $VncPassword = Get-RandomAlphanumericString -length 24
    Start-Process msiexec.exe -ArgumentList "/i $global:CloudRIGInstallBaseDir\Apps\TightVNC.msi /quiet /norestart ADDLOCAL=Server SET_USECONTROLAUTHENTICATION=1 VALUE_OF_USECONTROLAUTHENTICATION=1 SET_CONTROLPASSWORD=1 VALUE_OF_CONTROLPASSWORD=$VncPassword SET_USEVNCAUTHENTICATION=1 VALUE_OF_USEVNCAUTHENTICATION=1 SET_PASSWORD=1 VALUE_OF_PASSWORD=$VncPassword" -Wait
...
Function Install-VBCable
{
    ...
    (Get-AuthenticodeSignature -FilePath "$global:CloudRIGInstallBaseDir\Apps\VBCABLE_Driver\vbaudio_cable64_win7.cat").SignerCertificate | Export-Certificate -Type CERT -FilePath "$global:CloudRIGInstallBaseDir\Apps\VBCABLE_Driver\vbcable.cer"
    Import-Certificate -FilePath "$global:CloudRIGInstallBaseDir\Apps\VBCABLE_Driver\vbcable.cer" -CertStoreLocation 'Cert:\LocalMachine\TrustedPublisher'
    & c:\gcloudrig\downloads\vbcable\
    Start-Process -FilePath "$global:CloudRIGInstallBaseDir\Apps\VBCABLE_Driver\VBCABLE_Setup_x64.exe" -ArgumentList '-h','-i','-H','-n' -wait
    Write-host "`  - Success!"
}
...
Function Install-ControllerDriver
{
    if ((gwmi win32_operatingsystem | % caption) -like '*Windows Server 2019*') {
        ...
        (New-Object System.Net.WebClient).DownloadFile("http://download.microsoft.com/download/6/9/4/69446ACF-E625-4CCF-8F56-58B589934CD3/Xbox360_64Eng.exe", "$global:CloudRIGInstallBaseDir\Drivers\Xbox360_64Eng.exe")
